...
% goal([]).
% A file incomplete is supplied below, defining a predicate initKB/1 that initilizes a dynamic predicate kb/1 to a list represeting a file, so that consulting
% this code, we have
...
% that implements A∗
% , returning a path to the goal node [] with minimal
% cost, given Node and KB. Among the clauses in incomplete is
